/* ---------------------------------------------------------------------------
   ...
   always works. An OMDb key is used only to upgrade ratings from TMDB's score to
   the real IMDb score; without it the app says which score it is showing rather
   than pretending.
   ...
   Pipeline:
     ...
     4. Real IMDb rating via OMDb when available, else TMDB's own score
     5. Official trailer from YouTube, verified through oEmbed

   Rotten Tomatoes has no public API, so RT is always returned as null and the
   UI shows a dash. It is never invented.
--------------------------------------------------------------------------- */
...
/* ---- step 4: real IMDb rating, optional ---- */
async function omdbFor(title, omdbKey) {
  if (!omdbKey) return null;
  const j = await getJSON(`https://www.omdbapi.com/?t=${encodeURIComponent(title)}&type=series&apikey=${encodeURIComponent(omdbKey)}`);
...
const TIME_BUDGET_MS = 38000;   // leave headroom under the function's 60s ceiling
const WANT = 12;
...
async function runDiscover({ omdbKey, since, knownIds, knownTitles, today }) {
  ...
  // 1 — browse every service and both kinds AT ONCE
  const jobs = [];
  for (const pid of Object.keys(PROVIDERS)) for (const kind of ['new', 'season']) jobs.push({ pid, kind });
  const results = await pmap(jobs, 8, j => browse(j.pid, j.kind, since, 1)
    .then(r => ({ ...j, rows: r }))
    .catch(() => ({ ...j, rows: [] })));

  const upstreamErrors = results.filter(r => !r.rows.length)
  ...
  const pool = new Map();
  for (let round = 0; pool.size < MAX_CANDIDATES; round++) {
    let added = false;
    for (const q of queues) {
      if (round < q.length && pool.size < MAX_CANDIDATES) {
        const c = q[round];
  ...
  if (!pool.size) {
    return { ok: true, since, found: 0, scanned: 0, shows: [], rejected: [], upstreamErrors,
             ratingSource: omdbKey ? 'IMDb where available' : 'TMDB',
  ...
  const rejected = [];
  const verified = (await pmap(candidates, 6, async c => {
  ...
  const shortlist = verified.slice(0, WANT);
  const kept = (await pmap(shortlist, 5, async v => {
    const base = baseTitle(v.c.title);
    const [om, trailer] = await Promise.all([
      omdbFor(base, omdbKey).catch(() => null),
    ...
    return {
      ...
      imdb: score,
      ratingSource: source,
      rt: null,                    // no public RT API — never invented
      season: v.target,
  ...
  return {
    ...
    ratingSource: omdbKey ? 'IMDb where available, else TMDB' : 'TMDB (add OMDB_API_KEY for real IMDb scores)',
...
function readKeys(env) {
  ...
  const notes = [];
  if (!omdbKey) notes.push('No OMDB_API_KEY set, so ratings shown are TMDB scores rather than IMDb.');
...
async function handle(query, env) {
  ...
  if (query.selftest) {
    ...
    return { status: 200, body: {
      ...
      imdbRatings: !omdbKey ? 'not configured — will show TMDB scores instead'
                 : omdbOk ? 'working' : `key rejected: ${(o && o.Error) || 'unknown'}`,
      notes,
    } };
  }
