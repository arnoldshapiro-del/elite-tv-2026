...
/* `floors` are the TMDB vote_average bars this browse asks for. They matter when
   the caller lowers the IMDb bar: TMDB would otherwise never hand back a show
   rated under 7 in the first place, so a request for "6.5 and up" would come
   home empty no matter how long it hunted. At the standing 7.5 bar the numbers
   work out to exactly 7 and 7.5 — the same URLs this has always sent. */
async function browse(providerId, kind, since, page, floors) {
  const f = floors || { new: 7, season: MIN_TMDB_SCORE };
  const common = `with_watch_providers=${providerId}&watch_region=US&page=${page}`;
  const q = kind === 'new'
    ? `first_air_date.gte=${since}&vote_average.gte=${f.new}&vote_count.gte=${MIN_VOTES}&sort_by=vote_average.desc&${common}`
    : `air_date.gte=${since}&first_air_date.lte=${since}&vote_average.gte=${f.season}&vote_count.gte=120&sort_by=vote_average.desc&${common}`;
...
/* Keep a caller-supplied number inside sane walls, and fall back to the standing
   value when it is missing or nonsense. Nobody gets to ask for a 0.0 bar or a
   500-show shortlist by editing the URL. */
...
async function runDiscover({ omdbKey, since, knownIds, knownTitles, today, minBar, wantN }) {
  // The bar he'll accept and how many to bring back. Left alone, these are
  // exactly the constants above, so an ordinary search behaves as it always has.
  const bar = minBar == null ? MIN_IMDB : clamp(minBar, 5.0, 9.0, MIN_IMDB);
  const want = wantN == null ? WANT : Math.round(clamp(wantN, 4, 20, WANT));
  const floors = { new: Math.min(7, bar), season: Math.min(MIN_TMDB_SCORE, bar) };
  const t0 = Date.now();
  ...
  const results = await pmap(jobs, 10, j => browse(j.pid, j.kind, since, j.page, floors)
    .then(r => ({ ...j, rows: r }))
    .catch(() => ({ ...j, rows: [] })));
  ...
  const kept = (await pmap(shortlist, 8, async v => {
    ...
    if (score !== null && score < bar) {
      rejected.push({ title: v.c.title, why: `IMDb ${score} below ${bar}` });
      return null;
    }
...
function parseParams(query) {
  ...
  // "Grow the collection" sends these two; every other caller sends neither, and
  // undefined means "use the standing bar and batch size".
  const b = Number(query.bar), w = Number(query.want);
  const minBar = isFinite(b) && query.bar !== '' && query.bar != null ? b : undefined;
  const wantN = isFinite(w) && query.want !== '' && query.want != null ? w : undefined;
  return { since, knownIds, knownTitles, minBar, wantN };
}
...
async function handle(query, env) {
  ...
  try {
    ...
    const { since, knownIds, knownTitles, minBar, wantN } = parseParams(query);
    const body = await runDiscover({ omdbKey, since, knownIds, knownTitles, today: query.today, minBar, wantN });
