...
function readKeys(env) {
  // Trim: a value pasted with a stray space or newline is a real and very
  // confusing failure mode, since the variable looks correctly set.
  const raw = n => (env[n] === undefined || env[n] === null) ? undefined : String(env[n]);
  const pick = (...names) => {
    for (const n of names) { const v = raw(n); if (v !== undefined) return { name: n, value: v }; }
    return { name: names[0], value: undefined };
  };
  const t = pick('TMDB_API_KEY', 'TMDB_KEY');
  const o = pick('OMDB_API_KEY', 'OMDB_KEY');

  const tmdbKey = (t.value || '').trim();
  const omdbKey = (o.value || '').trim();

  // Distinguish "never set" from "set but blank" — otherwise both look identical
  // in the dashboard and there is no way to tell what to fix.
  const diag = [];
  const check = (label, got, cleaned) => {
    if (got === undefined) diag.push(`${label} is not set on this project`);
    else if (!cleaned) diag.push(`${label} exists but its value is empty — open it and paste the key in, then redeploy`);
  };
  check('TMDB_API_KEY', t.value, tmdbKey);
  check('OMDB_API_KEY', o.value, omdbKey);

  const missing = [];
  if (!tmdbKey) missing.push('TMDB_API_KEY');
  if (!omdbKey) missing.push('OMDB_API_KEY');
  return { tmdbKey, omdbKey, missing, diag };
}
...
async function handle(query, env) {
  const { tmdbKey, omdbKey, missing, diag } = readKeys(env);
  if (missing.length) {
    return {
      status: 200,
      body: {
  ...
  // ?q=... — search the whole TMDB catalogue, not just the curated list
  if (query.q) {
    const q = String(query.q).slice(0, 120);
    const r = await getJSON(`${TMDB}/search/tv?api_key=${encodeURIComponent(tmdbKey)}` +
      `&query=${encodeURIComponent(q)}&include_adult=false&language=en-US&page=1`);
    if (!r.ok) {
      return { status: 200, body: { ok: false, code: 'SEARCH_FAILED',
        message: (r.json && r.json.status_message) || `TMDB search failed (HTTP ${r.status})` } };
    }
    const rows = (r.json.results || [])
      ...
      .map(x => ({
        tmdb: x.id,
        title: x.name,
        poster: x.poster_path.replace(/^\//, ''),
        genre: (x.genre_ids || []).map(g => GENRES[g]).filter(Boolean).slice(0, 2).join(', ') || 'Drama',
        tmdbScore: typeof x.vote_average === 'number' ? Math.round(x.vote_average * 10) / 10 : null,
        tmdbVotes: x.vote_count || 0,
        firstAir: x.first_air_date || '',
        overview: x.overview || '',
      }));

    // real IMDb rating for the top hits, so search results carry an honest score
    for (const row of rows.slice(0, 10)) {
      try {
        const info = await imdbRatingFor(tmdbKey, omdbKey, row.tmdb);
        row.imdb = info.imdb;
        row.imdbId = info.imdbId;
      } catch { row.imdb = null; }
    }
    return { status: 200, body: { ok: true, query: q, count: rows.length, shows: rows } };
  }
