/* ---------------------------------------------------------------------------
   ...
   WHERE EACH NUMBER COMES FROM — nothing here is invented:
     ...
     - IMDb user rating          -> IMDb's own ratings feed, key-free (OMDb
                                    enriches with plot/cast when a key exists)
...
/* The same rating with NO key at all: IMDb's own ratings feed — the endpoint
   IMDb's embeddable rating widgets read — answers for any tt-id with the real
   rating and vote count in a tiny JSONP wrapper. (The title PAGE sits behind a
   bot check that returns HTTP 202, so it cannot be read the way RT pages can;
   this feed is the surface IMDb leaves open.) Exact by id, so it cannot land
   on the wrong film. Any miss -> null -> "—", never a guess. */
async function imdbFromWidget(imdbId) {
  if (!imdbId || !/^tt\d+$/.test(imdbId)) return null;
  const r = await get('https://p.media-imdb.com/static-content/documents/v1/title/' +
    imdbId + '/ratings%3Fjsonp=imdb.rating.run:imdb.api.title.ratings/data.json');
  if (r.status !== 200) return null;
  const m = r.text.match(/\(([\s\S]*)\)\s*$/);
  if (!m) return null;
  try {
    const res = JSON.parse(m[1]).resource || {};
    const v = Number(res.rating);
    if (!Number.isFinite(v)) return null;
    return {
      imdb: v, imdbId, votes: Number(res.ratingCount) || 0,
      plot: null, actors: null, director: null, genre: null, rated: null,
      runtime: null, metascore: null, awards: null, boxOffice: null, released: null,
    };
  } catch { return null; }
}

/* OMDb first when a key is configured (it also carries plot/cast extras); the
   key-free feed when there is no key or OMDb came back without a rating. */
async function imdbUserRating(title, year, omdbKey, imdbId) {
  const om = await omdbMovie(title, year, omdbKey, imdbId).catch(() => null);
  if (om && om.imdb !== null) return om;
  const w = await imdbFromWidget(imdbId).catch(() => null);
  if (w && om) return { ...om, imdb: w.imdb, votes: w.votes || om.votes };
  return w || om;
}
...
async function runMovies({ mode, todayISO, omdbKey, days, minImdb, minRt, want }) {
  ...
  const scored = (await pmap(inWindow, 4, async m => {
    ...
    const [rt, om] = await Promise.all([
      ...
      imdbUserRating(m.title, year, omdbKey, ids.imdbId).catch(() => null),
    ]);
  ...
  return {
    ...
    ratingSource: omdbKey ? 'IMDb via OMDb, Rotten Tomatoes read from RT'
                          : 'IMDb read from its own ratings feed, Rotten Tomatoes read from RT',
...
/* ---------------------------------------------------------------------------
   STREAMING — "Best of Streaming" search (mode=stream).

   Finds the top-rated films CURRENTLY STREAMING on the five services, using
   the same keyless sources as everything above: TMDB's public browse pages
   filtered by watch provider (that availability data is JustWatch's, published
   by TMDB itself), Wikidata for the exact RT page + IMDb id, RT's own page for
   the Tomatometer, IMDb's own ratings feed for the IMDb score.

   THE BAR IS FIXED AND IS ARNIE'S: Tomatometer >= 85% OR IMDb >= 7.5. A film
   whose scores cannot be verified is rejected, never guessed through.

   The client sends `known` (every film already in his collection), so each
   press returns only films he has never been shown — that is what makes the
   "what's been added since last time" button honest. `truncated` + press-again
   works exactly like the TV side's discovery.
--------------------------------------------------------------------------- */
const STREAM_PROVIDERS = { 15: 'Hulu', 8: 'Netflix', 350: 'Apple TV', 9: 'Prime Video', 1899: 'HBO Max' };
const STREAM_MIN_IMDB = 7.5;
const STREAM_MIN_RT_CRIT = 85;

/* One public browse page for one service. `sort` decides what "new" means:
   vote_average finds the best of the whole catalogue, primary_release_date
   finds well-rated films that only just arrived. Both are used.
   HBO Max is provider id 1899 (the "Max" era id — still answering after the
   2025 rename back to HBO Max; the old 384 returns an empty page, probed
   2026-08-03). */
async function browseStream(providerId, { sort, page, floor, votes, toISO } = {}) {
  let q = 'watch_region=US&with_watch_providers=' + providerId +
    '&sort_by=' + (sort || 'vote_average.desc') +
    '&vote_average.gte=' + (floor != null ? floor : 7) +
    '&vote_count.gte=' + (votes != null ? votes : 300) +
    '&page=' + (page || 1);
  if (toISO) q += '&release_date.lte=' + toISO;
  const r = await get('https://www.themoviedb.org/movie?' + q);
  if (r.status !== 200) return [];
  return parseCards(r.text).map(c => ({ ...c, service: STREAM_PROVIDERS[providerId] || String(providerId) }));
}

/* Host-aware, like discover-core: Vercel allows 60s, Netlify far less. */
const STREAM_BUDGET_MS = process.env.VERCEL ? 45000 : 22000;

async function runStream({ todayISO, omdbKey, knownIds, providerIds, want }) {
  const t0 = Date.now();
  const outOfTime = () => Date.now() - t0 > STREAM_BUDGET_MS;
  const known = new Set((knownIds || []).map(Number).filter(Boolean));
  const WANT = want || 12;
  const pids = (providerIds && providerIds.length ? providerIds : Object.keys(STREAM_PROVIDERS))
    .map(Number).filter(p => STREAM_PROVIDERS[p]);

  // 1 — browse every service two ways at once
  const jobs = [];
  for (const pid of pids) {
    jobs.push({ pid, opts: { sort: 'vote_average.desc', page: 1, votes: 300 } });
    jobs.push({ pid, opts: { sort: 'vote_average.desc', page: 2, votes: 300 } });
    jobs.push({ pid, opts: { sort: 'primary_release_date.desc', page: 1, votes: 100, toISO: todayISO } });
  }
  const results = await pmap(jobs, 10, j => browseStream(j.pid, j.opts)
    .then(rows => ({ pid: j.pid, rows })).catch(() => ({ pid: j.pid, rows: [] })));

  /* Drop what he already has, merge the same film found on two services
     ("Hulu · HBO Max" — it really is on both), and interleave round-robin so
     one service cannot fill the whole result set (the TV side's lesson). */
  const byService = {};
  for (const r of results) {
    const q = (byService[r.pid] = byService[r.pid] || []);
    for (const c of r.rows) {
      if (known.has(c.tmdb)) continue;
      if (!q.find(x => x.tmdb === c.tmdb)) q.push(c);
    }
  }
  const queues = Object.values(byService);
  const pool = new Map();
  const CAP = Math.min(30, Math.max(WANT * 2, 16));
  for (let round = 0; pool.size < CAP; round++) {
    let added = false;
    for (const q of queues) {
      if (round < q.length && pool.size < CAP) {
        const c = q[round];
        const prev = pool.get(c.tmdb);
        if (prev) { if (!prev.service.includes(c.service)) prev.service += ' · ' + c.service; }
        else { pool.set(c.tmdb, { ...c }); added = true; }
      }
    }
    if (!added) break;
  }
  // A film pooled once but found again on a later service still gets credited.
  for (const q of queues) for (const c of q) {
    const prev = pool.get(c.tmdb);
    if (prev && !prev.service.includes(c.service)) prev.service += ' · ' + c.service;
  }

  if (!pool.size) {
    return { ok: true, mode: 'stream', today: todayISO, found: 0, scanned: 0,
             bar: { imdb: STREAM_MIN_IMDB, rtCritics: STREAM_MIN_RT_CRIT },
             movies: [], rejected: [], truncated: false,
             message: 'No films you don\'t already have turned up on those services just now.' };
  }

  // 2 — TMDB detail (year, poster, plot, cast) for the pooled slice
  const candidates = [...pool.values()];
  const detailed = (await pmap(candidates, 8, async c => {
    if (outOfTime()) return null;
    const d = await tmdbDetail(c.tmdb).catch(() => null);
    if (!d) return null;
    return { ...c, ...d, poster: d.poster || (c.posterFile ? TMDB_IMG + c.posterFile : null) };
  ...
  // 3 — exact ids for the whole batch in one query, then the real scores.
  const idMap = await wikidataIds(detailed.map(m => m.tmdb)).catch(() => ({}));
  const rejected = [];
  const scored = (await pmap(detailed, 4, async m => {
    if (outOfTime()) return null;
    const year = m.releaseISO ? +m.releaseISO.slice(0, 4) : null;
    const ids = idMap[String(m.tmdb)] || {};
    const [rt, om] = await Promise.all([
  ...
  for (const m of scored) {
    ...
    if (imdb === null && rtCrit === null) {
      rejected.push({ title: m.title, why: 'no verifiable IMDb or Tomatometer score' });
      continue;
    }
    if (!((imdb !== null && imdb >= STREAM_MIN_IMDB) || (rtCrit !== null && rtCrit >= STREAM_MIN_RT_CRIT))) {
      rejected.push({
        title: m.title,
        why: `below the bar (${imdb !== null ? 'IMDb ' + imdb : 'no IMDb'}, ` +
             `${rtCrit !== null ? 'Tomatometer ' + rtCrit + '%' : 'no Tomatometer'})`,
      });
      continue;
    }
    kept.push({ ...m, imdb, rtAud, rtCrit });
  }
  kept.sort((a, b) => ((b.imdb || 0) - (a.imdb || 0)) || ((b.rtCrit || 0) - (a.rtCrit || 0)));

  // 4 — trailers, only for what survived
  const shortlist = kept.slice(0, WANT);
  await pmap(shortlist, 6, async m => {
    if (outOfTime()) { m.trailer = null; return; }
    const year = m.releaseISO ? +m.releaseISO.slice(0, 4) : null;
  ...
  return {
    ok: true, mode: 'stream', today: todayISO,
    bar: { imdb: STREAM_MIN_IMDB, rtCritics: STREAM_MIN_RT_CRIT },
    services: pids.map(p => STREAM_PROVIDERS[p]),
    found: movies.length, scanned: candidates.length,
    elapsedMs: Date.now() - t0,
    truncated: kept.length > shortlist.length || candidates.length > detailed.length,
    movies, rejected: rejected.slice(0, 25),
  };
}
...
function readKeys(env) {
  ...
  const notes = [];
  if (!omdbKey) notes.push('No OMDB_API_KEY set — IMDb ratings are read key-free from IMDb\'s own ratings feed instead (OMDb would only add plot/cast extras).');
...
async function handle(query, env) {
  ...
  if (query.selftest) {
    ...
    return { status: 200, body: {
      ...
      imdbRatings: omdbKey
        ? ((o && o.Response !== 'False') ? 'working — via OMDb' : `key rejected: ${(o && o.Error) || 'unknown'}`)
        : ((w && w.imdb) ? 'working — IMDb\'s own ratings feed, no key needed'
                         : 'FAILED: IMDb ratings feed unreachable (and no OMDB_API_KEY set)'),
      streamingBrowse: sb.length
        ? `working — provider-filtered browse answering (HBO Max page 1: ${sb.length} films)`
        : 'FAILED: TMDB provider browse returned no cards (regex or provider id drift?)',
  ...
  try {
    ...
    if (query.mode === 'stream') {
      ...
      const providerIds = String(query.providers || '').split(',').map(s => s.trim()).filter(Boolean);
      const w = Number(query.want);
      const want = isFinite(w) && w ? Math.min(20, Math.max(4, Math.round(w))) : undefined;
      const body = await runStream({ todayISO, omdbKey, knownIds, providerIds, want });
...
module.exports = {
  handle, runMovies, runStream, browseStream, rtScoresFor, rtForSeries,
  wikidataIds, tmdbDetail, imdbFromWidget, imdbUserRating, trailerFor,
  parseCards, pmap, get,
  THEATRE, MIN_IMDB, MIN_RT_AUDIENCE,
  STREAM_PROVIDERS, STREAM_MIN_IMDB, STREAM_MIN_RT_CRIT,
};
